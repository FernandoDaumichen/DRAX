...
const SongList: React.FC<SongListProps> = ({ songs, togglePlayPause, playingSongId, audioRefs }) => {
  ...
  return (
    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {songs.map((item) => (
        <div key={item.id} className="rounded-lg shadow-lg p-4 flex items-center space-x-4" style={{ backgroundColor: "#6C53B9", color: "white" }}>
          <div className="flex-shrink-0">
            <Image src={item.album.cover} alt={`Album cover of ${item.album.title}`} width={100} height={100} className="rounded" />
          </div>
          <div className="flex-grow">
            ...
            <div className="relative" style={{ width: '50px', height: '50px' }}>
              <svg width="50" height="50" className="absolute top-0 left-0">
                <defs>
                  <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="100%">
                    ...
                    <stop offset="50%" style={{ stopColor: 'pink', stopOpacity: 1 }} />
                    <stop offset="100%" style={{ stopColor: 'white', stopOpacity: 1 }} />
                  </linearGradient>
                </defs>
                <circle cx="25" cy="25" r="20" fill="transparent" stroke="url(#gradient)" strokeWidth="10" strokeDasharray={calculateStrokeDasharray(progress[item.id] || 0)} />
              </svg>
              <button
              ...
              >
                <FontAwesomeIcon icon={playingSongId === item.id && !audioRefs.current[item.id]?.paused ? faPause : faPlay} style={{ fontSize: '20px' }} />
              </button>
            </div>
          </div>
        </div>
